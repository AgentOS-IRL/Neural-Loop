...
Codex chat flow with the same `create_task`, `create_sub_task`, and `Notes`
tool contract used by audio mode. A `create_task` call can include a
`sub_tasks` array, and the runner will print the parent payload followed by
each nested dummy subtask payload. Tool calls are handled by dummy executors
that print the task or note payload instead of saving into the app.
