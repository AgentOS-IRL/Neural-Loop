...
struct WatchRestTimerView: View {
    ...
    init(exerciseID: String, completedSetID: String, totalSeconds: Int, onComplete: ((String?) -> Void)? = nil) {
    ...
    var body: some View {
        ...
        .onAppear {
            // Re-resolve next set with injected store
            if let exercise = exercise {
                viewModel.nextSetID = WatchRestTimerViewModel.resolveNextIncompleteSet(
                    in: exercise, after: completedSetID
                )
            }
            viewModel.start()
        }
