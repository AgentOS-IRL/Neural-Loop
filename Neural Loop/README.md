...
## Key modules
- **App entry & navigation** – `Neural_LoopApp.swift` bootstraps the app, requests calendar access, registers `CompletedRecurringTask` in SwiftData, and injects `UnifiedDataModel.shared`. `ContentView.swift` wires the `AppTab` enum, custom `LiquidGlassTabBar`, and the four main tabs (goals, tasks, calendar, settings), with the tasks tab opening the merged hub for todo and habits.
...
- **Automation helpers** – `ai_or_not/auto_notification_scheduler/` contains `HabitAutoScheduler` and `WaterAutoScheduler`, which compute reminder windows before handing off scheduling to `NotificationManager.swift`. `NotificationManager` centralizes permission, logging, and one-off/repeating notification creation. `tab_bar/` defines `LiquidGlassTabBar`, `GlassTabItem`, and `AppTab` so the visual chrome stays modular, while `task_screen/TaskHubView.swift` hosts the merged todo/habit destination and its secondary selector.
